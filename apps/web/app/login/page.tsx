...
export default function LoginPage() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error?.message ?? payload?.error ?? "Login failed.");
      }
      ...
      <div className="relative z-10 mx-auto w-full max-w-md">
        <Card className="border-border/60 bg-card/95 p-7 shadow-2xl md:p-8">
          ...
          <form className="space-y-5" onSubmit={onSubmit}>
            ...
            <Button type="submit" size="lg" disabled={isSubmitting} className="w-full text-sm font-semibold p-3 h-fit rounded-2xl">
              <LogIn className="h-4 w-4" aria-hidden="true" />
              {isSubmitting ? "Signing in..." : "Sign in"}
            </Button>

            <p className="text-center text-sm text-muted-foreground">
              Need an account? <Link className="font-medium underline underline-offset-4" href="/register">Create one</Link>
            </p>
          </form>
        </Card>
      </div>
